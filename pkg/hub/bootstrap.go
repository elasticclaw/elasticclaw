...
cat > "$HOME/.claw-bridge.env" << 'CLAW_ENV_EOF'
ELASTICCLAW_HUB_URL=$ELASTICCLAW_HUB_URL
ELASTICCLAW_CLAW_ID=$ELASTICCLAW_CLAW_ID
ELASTICCLAW_CLAW_TOKEN=$ELASTICCLAW_CLAW_TOKEN
ELASTICCLAW_CLAW_NAME=$ELASTICCLAW_CLAW_NAME
ELASTICCLAW_GATEWAY_PASSWORD=$ELASTICCLAW_GATEWAY_PASSWORD
CLAW_ENV_EOF
# Expand the variables (heredoc above is literal, so re-write with actual values)
cat > "$HOME/.claw-bridge.env" <<EOF
ELASTICCLAW_HUB_URL=$ELASTICCLAW_HUB_URL
ELASTICCLAW_CLAW_ID=$ELASTICCLAW_CLAW_ID
ELASTICCLAW_CLAW_TOKEN=$ELASTICCLAW_CLAW_TOKEN
ELASTICCLAW_CLAW_NAME=$ELASTICCLAW_CLAW_NAME
