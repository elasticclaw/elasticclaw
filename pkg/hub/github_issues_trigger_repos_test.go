...
func TestGitHubIssuesWebhookFallsBackToLegacyRepos(t *testing.T) {
	ghi := factorytest.NewMockGitHubIssues(t)
	ghi.WebhookSecret = "test-webhook-secret"
	li := factorytest.NewMockLinear(t)
	...
	if factoryName != "legacy-org-b" {
		t.Fatalf("factory_name = %q, want legacy-org-b", factoryName)
	}
}
