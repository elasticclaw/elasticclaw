...
func TestConvertWorkflowV1ToV2WarnsAboutSessionResume(t *testing.T) {
	for _, configured := range []bool{false, true} {
		name := "omitted"
		data := sampleWorkflowV1
		if configured {
			name = "configured"
			data += "\nsession_resume:\n  read_files: [NOTES.md]\n  state_check: Read the latest notes\n"
		}
		t.Run(name, func(t *testing.T) {
			res, err := convert.Convert(convert.KindWorkflow, []byte(data), convert.Options{To: "2"})
			if err != nil {
				t.Fatalf("convert: %v", err)
			}
			warning := "session_resume: not represented in workflow v2 schema yet"
			...
			if _, err := v2.ParseAndValidateWorkflow(res.Output); err != nil {
				t.Fatalf("converted workflow invalid: %v", err)
			}
		})
	}
}
