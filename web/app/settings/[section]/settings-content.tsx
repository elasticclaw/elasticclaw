...
import { fetchModels, type ModelsResponse, type ProviderInfo } from "@/lib/api"
...
function LLMSection({ settings, onSave, saving }: { settings: SettingsData; onSave: (p: object) => void; saving: boolean }) {
  ...
  useEffect(() => {
    ...
    fetchModels()
      .then(data => { if (!cancelled) setModelsData(data) })
      .catch(() => { /* silently fail — fallback to free input */ })
      .finally(() => { if (!cancelled) setModelsLoading(false) })
    return () => { cancelled = true }
  }, [])

  // Build a map of provider -> models from the API response, merged with hardcoded fallback
  const providerModelsMap = React.useMemo(() => {
    const map: Record<string, { id: string; name: string }[]> = {}
    if (modelsData?.providers) {
      for (const p of modelsData.providers) {
        map[p.name] = p.models.map(m => ({ id: m.id, name: m.name }))
      }
    }
    return map
  }, [modelsData])
  ...
  // Get models for current provider selection (handles custom providers too)
  const currentProviderModels = providerModelsMap[newProvider === "other" ? newCustomProvider : newProvider]
    ...
    <div>
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        <div>
          ...
          {modelsLoading ? (
            <div className="h-8 text-sm w-full rounded-md border border-input bg-background px-2 py-1 flex items-center text-muted-foreground">
              Loading models…
            </div>
          ) : currentProviderModels && currentProviderModels.length > 0 ? (
            <select
