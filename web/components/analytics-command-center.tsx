    ...
    async (cursor?: string, append = false) => {
      ...
      try {
        ...
        const [
          ...
          fetchAnalyticsCostDrivers(effectiveFilters, filters.factory ? "workflow" : "factory", { signal: controller.signal }),
  ...
  return (
    <main className="h-full overflow-auto bg-background">
      <div className="mx-auto max-w-[1400px] space-y-5 p-5 lg:p-6">
        ...
        <CostDrivers drivers={drivers} filters={filters} setFilters={setFilters} />
        <RunsTable
          runs={runs}
...
function CostDrivers({ drivers, filters, setFilters }: { drivers: AnalyticsCostDriver[]; filters: TaskRunAnalyticsFilters; setFilters: (updates: Record<string, string | undefined>) => void }) {
  const isWorkflowDrill = Boolean(filters.factory)
  const isLeaf = isWorkflowDrill && Boolean(filters.workflow)
  const subtitle = !isWorkflowDrill ? "By factory" : isLeaf ? `Workflow ${filters.workflow}` : `Workflows in ${filters.factory}`

  return <section className="rounded-lg border bg-card p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><h2 className="text-sm font-semibold">Top cost drivers</h2><p className="text-xs text-muted-foreground">{subtitle}</p></div>{isWorkflowDrill && <Button variant="ghost" size="sm" onClick={() => setFilters({ factory: undefined, workflow: undefined })}>← All factories</Button>}</div><Table><TableHeader><TableRow><TableHead>Name</TableHead><TableHead className="text-right">Runs</TableHead><TableHead className="text-right">Success</TableHead><TableHead className="text-right">Cost</TableHead><TableHead className="text-right">Cost / merged PR</TableHead><TableHead /></TableRow></TableHeader><TableBody>{drivers.slice(0, 10).map((driver) => <TableRow key={driver.name} className={isLeaf ? undefined : "cursor-pointer"} onClick={isLeaf ? undefined : () => setFilters(isWorkflowDrill ? { workflow: driver.name } : { factory: driver.name })}><TableCell className="font-medium">{driver.name}</TableCell><TableCell className="text-right tabular-nums">{driver.runs}</TableCell><TableCell className="text-right tabular-nums">{formatPercent(driver.successRate)}</TableCell><TableCell className="text-right tabular-nums">{usd.format(driver.costUsd)}</TableCell><TableCell className="text-right tabular-nums">{usd.format(driver.costPerMergedPr)}</TableCell><TableCell>{!isLeaf && <ChevronRight className="size-4 text-muted-foreground" />}</TableCell></TableRow>)}</TableBody></Table></section>
}
