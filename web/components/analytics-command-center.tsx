...
import { KpiCell, KpiGrid } from "@/components/ui/kpi"
import {
  Select,
  SelectContent,
  SelectItem,
...
const tooltipContentClassName = "max-w-xs border border-border bg-popover text-foreground shadow-lg"
const tooltipArrowClassName = "bg-popover fill-popover"
const chartConfig = {
  clean: { label: "Clean", color: "var(--chart-positive)" },
  humanInTheLoop: { label: "Human on the loop", color: "var(--chart-attention)" },
  warning: { label: "Warning", color: "var(--chart-warning)" },
  failed: { label: "Failed", color: "var(--chart-negative)" },
  ticketDelivered: { label: "Delivered", color: "var(--chart-positive)" },
  ticketInProgress: { label: "In progress", color: "var(--chart-attention)" },
  ticketFailed: { label: "Failed", color: "var(--chart-negative)" },
  ...
  return (
    <main className="h-full overflow-auto bg-background">
      <div className="mx-auto max-w-[1400px] space-y-5 p-5 lg:p-6">
        <header>
          <Kicker emphasis>Operations intelligence</Kicker>
...
function FilterBar({
  ...
  return (
    <div className="flex flex-wrap gap-2 border border-border p-2">
      <div className="w-56">
        <WorkspaceSelect
...
function Heatmap({ heatmap, maxCost, selectedDay, onSelectDay, onClearSelectedDay }: { heatmap: ReturnType<typeof useHeatmap>; maxCost: number; selectedDay?: string; onSelectDay: (day: string) => void; onClearSelectedDay: () => void }) {
  ...
  return <section ref={containerRef} className="blueprint relative p-4"><div className="mb-4 flex items-center gap-1"><div><h4>Cost by day</h4><p className="text-[11px] text-muted-foreground">Each square is a day; darker means more spend.</p></div><InfoTooltip text="Each square is a day; darker means more spend. Click a day to focus the whole page on it." />{selectedDayLabel && <span className="ml-2 inline-flex items-center gap-1 border border-border bg-neutral-100 px-2 py-0.5 text-xs text-muted-foreground">{selectedDayLabel}<button type="button" aria-label={`Clear ${selectedDayLabel} filter`} onClick={onClearSelectedDay} className="px-0.5 text-foreground hover:bg-accent">×</button></span>}</div><div className="flex gap-2"><div className="flex w-6 flex-col text-center font-mono text-[10px] text-muted-foreground"><div className="mb-1 h-4" aria-hidden="true" /><div className="grid flex-1 grid-rows-7 gap-1">{["M", "T", "W", "T", "F", "S", "S"].map((day, index) => <span key={`${day}-${index}`} className="flex items-center justify-center">{day}</span>)}</div></div><div className="min-w-0 flex-1"><div className="relative mb-1 h-4 font-mono text-[10px] text-muted-foreground">{heatmap.monthLabels.map(({ week, label }) => <span key={`${week}-${label}`} className="absolute" style={{ left: `${(week / 52) * 100}%` }}>{label}</span>)}</div><div className="grid grid-flow-col grid-cols-[repeat(52,minmax(0,1fr))] grid-rows-7 gap-1">{heatmap.days.map(({ iso, point }) => { const level = point?.costUsd ? Math.min(5, Math.ceil((point.costUsd / maxCost) * 5)) : 0; const selected = iso === selectedDay; return <button key={iso} onClick={() => onSelectDay(iso)} onMouseEnter={(event) => showTooltip(iso, point, event.clientX, event.clientY)} onMouseMove={(event) => showTooltip(iso, point, event.clientX, event.clientY)} onMouseLeave={() => setTooltip(null)} onFocus={(event) => { const rect = event.currentTarget.getBoundingClientRect(); showTooltip(iso, point, rect.left + rect.width / 2, rect.top + rect.height / 2) }} onBlur={() => setTooltip(null)} className={`aspect-square cursor-pointer border border-border transition-shadow hover:ring-2 hover:ring-ring hover:ring-offset-1 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 ${selected ? "ring-2 ring-primary ring-offset-1" : ""}`} style={{ background: level ? `var(--heatmap-${level})` : "var(--muted)" }} /> })}</div></div></div>{tooltip && <div role="tooltip" className="pointer-events-none absolute z-10 border border-border bg-popover px-2 py-1.5 text-xs text-foreground shadow-lg" style={{ left: tooltip.x, top: tooltip.y }}><div>{new Intl.DateTimeFormat(undefined, { weekday: "short", month: "short", day: "numeric", year: "numeric" }).format(new Date(`${tooltip.iso}T00:00:00`))}</div><div className="text-muted-foreground">{usdWhole.format(tooltip.point?.costUsd ?? 0)} · {tooltip.point?.runCount ?? 0} runs</div></div>}<div className="mt-3 flex justify-end gap-1 text-xs text-muted-foreground">Less {Array.from({ length: 5 }, (_, index) => <i key={index} className="size-3" style={{ background: `var(--heatmap-${index + 1})` }} />)} More</div></section>
}
...
const chartAxisTick = { fill: "var(--muted-foreground)", fontSize: 11, fontFamily: "var(--font-mono)" }
const chartGrid = { stroke: "var(--border)" }
function DailyCostChart({ costs, modelData }: { costs?: CostOverview; modelData: Record<string, string | number>[] }) { const labels = [...(costs?.seriesByModel ?? []).slice(0, 4).map((item) => item.model), "Other"]; return <ChartContainer config={chartConfig} className="h-64 w-full"><BarChart data={modelData}><CartesianGrid vertical={false} {...chartGrid} /><XAxis dataKey="date" tickFormatter={formatDate} tick={chartAxisTick} /><YAxis tick={chartAxisTick} /><ChartTooltip content={<ChartTooltipContent />} /><Legend formatter={legendTextFormatter} />{labels.map((label, index) => <Bar key={label} dataKey={label} name={label} stackId="cost" fill={`var(--chart-${index + 1})`} radius={0} />)}</BarChart></ChartContainer> }
function WorkflowCostComparisonChart({ drivers }: { drivers: AnalyticsCostDriver[] }) { const { data, topNames } = useWorkflowCostComparisonData(drivers); if (drivers.length === 0) return <p className="py-8 text-center text-sm text-muted-foreground">No cost data for this period.</p>; return <ChartContainer config={chartConfig} className="h-64 w-full"><LineChart data={data}><CartesianGrid vertical={false} {...chartGrid} /><XAxis dataKey="date" tickFormatter={formatDate} tick={chartAxisTick} /><YAxis tick={chartAxisTick} /><ChartTooltip content={<ChartTooltipContent />} /><Legend formatter={legendTextFormatter} />{topNames.map((name, index) => <Line key={name} type="monotone" dataKey={name} name={name} stroke={`var(--chart-${index + 1})`} dot={false} strokeWidth={2} />)}{drivers.length > 5 && <Line type="monotone" dataKey="Other" name="Other" stroke="var(--chart-5)" dot={false} strokeWidth={2} />}</LineChart></ChartContainer> }
function OutcomesChart({ effect }: { effect?: AnalyticsEffectiveness }) { return <ChartContainer config={chartConfig} className="h-64 w-full"><BarChart data={effect?.outcomesByDay}><CartesianGrid vertical={false} {...chartGrid} /><XAxis dataKey="date" tickFormatter={formatDate} tick={chartAxisTick} /><YAxis allowDecimals={false} tick={chartAxisTick} /><ChartTooltip content={<ChartTooltipContent />} /><Legend formatter={legendTextFormatter} /><Bar dataKey="clean" name="Clean" stackId="outcome" fill="var(--chart-positive)" radius={0} /><Bar dataKey="humanInTheLoop" name="Human on the loop" stackId="outcome" fill="var(--chart-attention)" radius={0} /><Bar dataKey="warning" name="Warning" stackId="outcome" fill="var(--chart-warning)" radius={0} /><Bar dataKey="failed" name="Failed" stackId="outcome" fill="var(--chart-negative)" radius={0} /></BarChart></ChartContainer> }
function DeliveryFunnel({ effect }: { effect?: AnalyticsEffectiveness }) {
  ...
  return <div className="space-y-3 pt-3">{stages.map(([name, label], index) => {
    ...
    return <div key={name}><div className="mb-1 flex justify-between text-sm"><span>{label}</span><span className="font-mono tabular-nums">{value} {index ? `(${formatPercent(previous ? value / previous : undefined)})` : ""}</span></div><div className="h-3 bg-neutral-100"><div className={cn("h-full", index === 0 ? "bg-chart-1" : index === 1 ? "bg-chart-1/70" : "bg-chart-1/45")} style={{ width: `${Math.min(100, (value / (effect?.funnel.agentStarted || 1)) * 100)}%` }} /></div></div>
  })}</div>
}
function TicketThroughputChart({ effect }: { effect?: AnalyticsEffectiveness }) { if (!effect?.ticketsByDay?.length) return <p className="py-8 text-center text-sm text-muted-foreground">No ticket data for this period.</p>; return <ChartContainer config={chartConfig} className="h-64 w-full"><BarChart data={effect.ticketsByDay}><CartesianGrid vertical={false} {...chartGrid} /><XAxis dataKey="date" tickFormatter={formatDate} tick={chartAxisTick} /><YAxis allowDecimals={false} tick={chartAxisTick} /><ChartTooltip content={<ChartTooltipContent />} /><Legend formatter={legendTextFormatter} /><Bar dataKey="delivered" name="Delivered" stackId="ticket" fill="var(--chart-positive)" radius={0} /><Bar dataKey="inProgress" name="In progress" stackId="ticket" fill="var(--chart-attention)" radius={0} /><Bar dataKey="failed" name="Failed" stackId="ticket" fill="var(--chart-negative)" radius={0} /></BarChart></ChartContainer> }
function RunsPerTicketChart({ effect }: { effect?: AnalyticsEffectiveness }) { if (!effect?.runsPerTicket?.length) return <p className="py-8 text-center text-sm text-muted-foreground">No ticket data for this period.</p>; return <ChartContainer config={chartConfig} className="h-64 w-full"><BarChart data={effect.runsPerTicket} layout="vertical" margin={{ right: 36 }}><CartesianGrid horizontal={false} {...chartGrid} /><XAxis type="number" allowDecimals={false} tick={chartAxisTick} /><YAxis type="category" dataKey="bucket" width={40} tick={chartAxisTick} /><Bar dataKey="tickets" fill="var(--chart-1)" radius={0}><LabelList dataKey="tickets" position="right" /></Bar></BarChart></ChartContainer> }
...
function TopTicketsByCostChart({ effect }: { effect?: AnalyticsEffectiveness }) { if (!effect?.topTicketsByCost?.length) return <p className="py-8 text-center text-sm text-muted-foreground">No ticket data for this period.</p>; return <ChartContainer config={chartConfig} className="h-64 w-full"><BarChart data={effect.topTicketsByCost} layout="vertical" margin={{ right: 36 }}><CartesianGrid horizontal={false} {...chartGrid} /><XAxis type="number" tickFormatter={(value) => usdWhole.format(value)} tick={chartAxisTick} /><YAxis type="category" dataKey="issueId" width={90} interval={0} tick={chartAxisTick} tickFormatter={(id) => (id.length > 14 ? id.slice(0, 13) + "…" : id)} /><ChartTooltip content={<TopTicketTooltip />} /><Bar dataKey="costUsd" fill="var(--chart-1)" radius={0}><LabelList dataKey="costUsd" position="right" formatter={(value: number) => usd.format(value)} /></Bar></BarChart></ChartContainer> }
function CostPerMergedPrChart({ effect }: { effect?: AnalyticsEffectiveness }) { return <ChartContainer config={chartConfig} className="h-64 w-full"><LineChart data={effect?.costPerMergedPr.weekly}><CartesianGrid vertical={false} {...chartGrid} /><XAxis dataKey="weekStart" tickFormatter={formatDate} tick={chartAxisTick} /><YAxis tick={chartAxisTick} /><ChartTooltip content={<ChartTooltipContent />} /><ReferenceLine y={effect?.costPerMergedPr.average} stroke="var(--color-neutral-600)" strokeDasharray="4 4" /><Line type="monotone" dataKey="costPerMergedPr" name="Cost per merged PR" stroke="var(--chart-1)" dot={false} /></LineChart></ChartContainer> }
function CostDrivers({ drivers }: { drivers: AnalyticsCostDriver[] }) {
  return <Blueprint className="p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><div className="flex items-center gap-1"><h4>Top cost drivers</h4><InfoTooltip text="Where the money goes: total spend and efficiency per workflow in the selected period." /></div><p className="text-[11px] text-muted-foreground">By workflow</p></div></div><Table><TableHeader><TableRow><TableHead>Workflow</TableHead><TableHead className="text-right">Runs</TableHead><TableHead className="text-right">Success</TableHead><TableHead className="text-right">Cost</TableHead><TableHead className="text-right">Cost / merged PR</TableHead></TableRow></TableHeader><TableBody>{drivers.slice(0, 10).map((driver) => <TableRow key={driver.name}><TableCell className="font-medium">{driver.name}</TableCell><TableCell className="text-right tabular-nums">{driver.runs}</TableCell><TableCell className="text-right tabular-nums">{formatPercent(driver.successRate)}</TableCell><TableCell className="text-right tabular-nums">{usd.format(driver.costUsd)}</TableCell><TableCell className="text-right tabular-nums">{usd.format(driver.costPerMergedPr)}</TableCell></TableRow>)}</TableBody></Table></Blueprint>
}
const runsTableRightAlignedHeaders = new Set(["Cost", "Duration", "Start date"])
function RunsTable({ runs, page, canGoPrevious, canGoNext, onSelect, onPrevious, onNext }: { runs: TaskRunSummary[]; page: number; canGoPrevious: boolean; canGoNext: boolean; onSelect: (runId: string) => void; onPrevious: () => void; onNext: () => void }) { return <Blueprint className="p-4"><h4 className="mb-3">Runs</h4><Table><TableHeader><TableRow>{["Status", "Ticket", "Model", "Factory/Workflow", "Cost", "Duration", "Start date"].map((label) => <TableHead key={label} className={runsTableRightAlignedHeaders.has(label) ? "text-right" : ""}>{label}</TableHead>)}</TableRow></TableHeader><TableBody>{runs.map((run) => <TableRow key={run.runId} className="cursor-pointer" onClick={() => onSelect(run.runId)}><TableCell><StatusBadge status={run.status} /></TableCell><TableCell>{run.issueId || "—"}</TableCell><TableCell>{run.model || "—"}</TableCell><TableCell>{run.factoryName || run.workflowName || "—"}</TableCell><TableCell className="text-right tabular-nums">{usd.format(run.estimatedCostUsd || 0)}</TableCell><TableCell className="text-right tabular-nums">{formatDuration(run.finishedAt ? run.finishedAt - run.startedAt : undefined)}</TableCell><TableCell className="text-right tabular-nums">{run.startedAt ? new Date(run.startedAt).toLocaleDateString() : "—"}</TableCell></TableRow>)}</TableBody></Table><div className="mt-3 flex items-center justify-center gap-3"><Button variant="outline" size="sm" disabled={!canGoPrevious} onClick={onPrevious}>Previous</Button><span className="text-sm text-muted-foreground">Page {page}</span><Button variant="outline" size="sm" disabled={!canGoNext} onClick={onNext}>Next</Button></div></Blueprint> }
...
function KpiGroup({ title, columns = "sm:grid-cols-5", children }: { title: string; columns?: string; children: ReactNode }) { return <div><Kicker className="mb-2 block">{title}</Kicker><KpiGrid className={`grid-cols-2 ${columns}`}>{children}</KpiGrid></div> }
function Kpi({ label, value, change, good, cost, onClick, title }: { label: string; value?: string | number; change?: number; good?: boolean; cost?: boolean; onClick?: () => void; title?: string }) {
  ...
  const button = (
    <KpiCell role="button" tabIndex={disabled ? -1 : 0} onClick={onClick} onKeyDown={(event) => { if (!disabled && (event.key === "Enter" || event.key === " ")) onClick?.() }} className="cursor-pointer text-left hover:bg-accent" label={<>{label}{title && <Info aria-hidden className="ml-1 inline size-3.5 text-muted-foreground" />}</>} value={value ?? "—"} delta={change == null ? " " : <>{change > 0 ? "+" : ""}{(change * 100).toFixed(1)}% <span className="text-muted-foreground">vs prior</span></>} deltaTone={bad ? "muted" : "accent"} />
  )
...
function ChartCard({ title, info, children }: { title: string; info?: string; children: ReactNode }) { return <Blueprint className="p-4"><div className="mb-3 flex items-start gap-1"><div><h4>{title}</h4><p className="text-[11px] text-muted-foreground">{info}</p></div>{info && <InfoTooltip text={info} />}</div>{children}</Blueprint> }
