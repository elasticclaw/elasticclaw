...
export function TicketDetailPanel({ ticket, onClose, onOpenRun }: { ticket: AnalyticsTicket | null; onClose: () => void; onOpenRun: (run: AnalyticsTicketRunSummary) => void }) {
  ...
  const outcome = delivered ? "Work was delivered through a merged pull request." : current.status === "pr_open" ? `Work is done and waiting on a human: ${open.length} pull request${open.length === 1 ? "" : "s"} open, none merged or closed yet.` : current.status === "in_progress" ? "Work is currently in progress." : "The work did not reach delivery."
  return <>
