...
func TestActivityAndDigestRedaction(t *testing.T) {
	...
	for _, tt := range []struct{ input, activity, digest string }{
		{"Run: GITHUB_TOKEN=SECRET gh pr list", "Run: GITHUB_TOKEN=[redacted] gh pr list", "[redacted]"},
		{"- run: GITHUB_TOKEN=SECRET", "- run: GITHUB_TOKEN=[redacted]", "[redacted]"},
		{"cookie: token=SECRET; other=1", "cookie: token=[redacted] other=1", "[redacted]"},
		...
		{`GH_TOKEN='SECRET'`, `GH_TOKEN=[redacted]`, `[redacted]`},
		{`GH_TOKEN="SECRET`, `GH_TOKEN=[redacted]`, `[redacted]`},
		{`api_key="first\"SECRET" path=main.go`, `api_key=[redacted]SECRET" path=main.go`, `[redacted]`},
		{`api_key='first\'SECRET' path=main.go`, `api_key=[redacted]SECRET' path=main.go`, `[redacted]`},
		{`curl -d "{\"api_key\":\"first\\\"SECRET\",\"path\":\"main.go\"}"`, `curl -d "{\"api_key\":\"first\\\"SECRET\",\"path\":\"main.go\"}"`, `[redacted]`},
		...
		{"echo ghp_SECRET | gh auth login --with-token", "echo ghp_SECRET | gh auth login --with-token", "[redacted]"},
